...
import { Brain, Languages, Users, AlertCircle, Sparkles } from "lucide-react";
...
export default function ResearchSandbox({
  theme,
  onTaskChange,
}: ResearchSandboxProps) {
  const [activeTab, setActiveTab] = useState<
    "eeg" | "bilingual" | "marl" | "biases"
  >("eeg");
  const isDark = theme === "dark";

  // State for Widget 1: EEG Simulator
  const [eegTask, setEegTask] = useState<
    "resting" | "nback" | "switching" | "interpretable"
  >("resting");
  const [modelType, setModelType] = useState<"CNN" | "LSTM" | "Transformer">(
    "Transformer",
  );
  ...
  const [inputText, setInputText] = useState(
    "Currently engaged in machine learning research.",
  );
  const [saccCost, setSaccCost] = useState(0.12);
  const [switchTrajectory, setSwitchTrajectory] = useState<number[]>(
    Array(15).fill(0.1),
  );
  ...
  const [activeBias, setActiveBias] = useState<
    "Normal" | "Loss-Averse" | "Anchoring" | "Confirmation" | "Optimistic"
  >("Loss-Averse");
  ...
  useEffect(() => {
    ...
    const taskImpact =
      eegTask === "resting" ? 0.05 : eegTask === "nback" ? -0.04 : 0.01;
  ...
  useEffect(() => {
    ...
    const finalCost =
      textLower.length === 0
        ? 0
        : Math.min(0.98, 0.08 + switchFactor + words.length * 0.005);
    ...
    for (let i = 0; i < 15; i++) {
      const stepCost =
        i > 3 && i < 11
          ? finalCost * (1 + Math.sin((i - 4) * 0.4) * 0.3)
  ...
  useEffect(() => {
    ...
    const runSimulation = () => {
      ...
      setCoordinationRate(
        Math.min(
          100,
          Math.max(5, baseCoord + Math.floor((Math.random() - 0.5) * 8)),
        ),
      );
      setTrustLevel(
        Math.min(
          100,
          Math.max(5, baseTrust + Math.floor((Math.random() - 0.5) * 6)),
        ),
      );
  ...
  useEffect(() => {
    ...
    const runSimulation = () => {
      ...
      if (activeBias === "Normal") {
        winR = 85;
        expR = 40;
        status = "Converged";
      } else if (activeBias === "Loss-Averse") {
        winR = 92;
        expR = 30;
        status = "Optimal Avoidance";
      } else if (activeBias === "Anchoring") {
        winR = 65;
        expR = 15;
        status = "Stuck (Local Optima)";
      } else if (activeBias === "Confirmation") {
        winR = 40;
        expR = 10;
        status = "Failed to Learn";
      } else if (activeBias === "Optimistic") {
        winR = 90;
        expR = 80;
      ...
      if (feedbackActive) {
        if (activeBias === "Normal" || activeBias === "Optimistic")
          expR = 95; // highly responsive
        else if (activeBias === "Confirmation") {
          winR -= 10;
          status = "Rejecting Feedback";
        } // stubborn
        else if (activeBias === "Anchoring") {
          expR += 5;
        } // barely reacts
      ...
      setWinRate(
        Math.min(
          100,
          Math.max(0, winR + Math.floor((Math.random() - 0.5) * 5)),
        ),
      );
      setExplorationRate(
        Math.min(
          100,
          Math.max(0, expR + Math.floor((Math.random() - 0.5) * 5)),
        ),
      );
      ...
      <div className="flex flex-col xl:flex-row xl:items-end justify-between mb-8 gap-6">
        <div>
          <div className="flex items-center gap-2 mb-2">
            <span
              className={`flex items-center gap-1.5 px-2.5 py-0.5 text-xs font-mono rounded-full ${
                activeTab === "eeg"
                  ? isDark
          ...
          <p
            className={`text-sm mt-1 max-w-xl ${isDark ? "text-slate-400" : "text-slate-600"}`}
          >
            Interact directly with George&apos;s primary research focus areas.
            Toggle parameters, feed real text, and evaluate cognitive metrics in
            real-time.
          </p>
        </div>
        ...
        {/* Tab Controls */}
        <div
          className={`flex w-full xl:w-auto overflow-x-auto hide-scrollbar shrink-0 p-1 rounded-xl ${isDark ? "bg-slate-950 border border-slate-800" : "bg-slate-100 border border-slate-200"}`}
        >
          <button
            ...
            className={`shrink-0 flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-mono transition-all ${
              activeTab === "eeg"
                ? isDark
                  ...
                  : "bg-white text-orange-600 shadow-xs font-medium"
          ...
          {activeTab === "eeg" && (
            ...
            >
              ...
              <div className="lg:col-span-4 space-y-6">
                <div>
                  <h4
                    className={`text-sm font-mono font-medium mb-3 uppercase tracking-wider ${
                    ...
                    {[
                      {
                        id: "resting",
                        label: "Resting State",
                        desc: "Minimal cognitive load baseline",
                      },
                      {
                        id: "interpretable",
                        label: "Cardiac LIME/SHAP",
                        desc: "Analyzing explainable decision paths",
                      },
                      {
                        id: "switching",
                        label: "Bilingual Code-Switch",
                        desc: "High-demand language transition",
                      },
                      {
                        id: "nback",
                        label: "Visual N-Back Task",
                        desc: "DQN-robustness under severe load",
                      },
                    ].map((t) => (
                      <button
                        key={t.id}
              ...
              <div className="lg:col-span-8 flex flex-col justify-between">
                ...
                >
                  <div className="flex items-center justify-between mb-4 pb-3 border-b border-dashed border-slate-800/20">
                    <span className="text-xs font-mono text-slate-400">
                      SEED-VIG EXPERIMENTATION ENGINE
                    </span>
                    ...
                    <div className="w-full md:w-1/2 flex justify-center relative">
                      ...
                      >
                        ...
                        >
                          {/* Brain outline */}
                          <path
                            d="M 60,8 C 45,5 25,12 18,30 C 10,48 10,70 20,88 C 30,105 45,112 60,110 C 75,112 90,105 100,88 C 110,70 110,48 102,30 C 95,12 75,5 60,8 Z"
                            fill={
                              isDark
                                ? "rgba(15,23,42,0.4)"
                        ...
                        <motion.circle
                          ...
                          animate={
                            eegTask === "nback"
                              ? {
                                  scale: stimulusActive ? 1.8 : [1, 1.3, 1],
                                  opacity: [1, 0.8, 1],
                                }
                              : { scale: 1, opacity: 1 }
                          }
                          transition={{
                            duration: stimulusActive ? 0.2 : 1.5,
                            repeat: stimulusActive ? 0 : Infinity,
                          }}
                        />
                        <motion.circle
                          cx="35"
                        ...
                        {eegTask !== "resting" && (
                          <motion.path
                            d={
                              eegTask === "nback"
                                ? "M 60,22 Q 60,50 60,80"
                                : eegTask === "switching"
                                  ? "M 35,45 Q 60,60 85,45"
                                  : "M 25,70 Q 60,50 95,70"
                            }
                            fill="none"
                            stroke={
                              eegTask === "nback"
                            ...
                            transition={{
                              duration: stimulusActive ? 0.3 : 0.8,
                              repeat: Infinity,
                              ease: "linear",
                            }}
                          />
                      ...
                      <span className="absolute text-[8px] font-mono text-slate-400 top-2 left-2">
                        FRONTAL (Fz)
                      </span>
                      <span className="absolute text-[8px] font-mono text-slate-400 bottom-2 right-2">
                        OCCIPITAL (Oz)
                      </span>
                    </div>
                    ...
                    <div className="w-full md:w-1/2 space-y-4">
                      <div>
                        <div className="flex justify-between text-xs font-mono text-slate-400 mb-1">
                          ...
                          <span
                            className={`font-medium ${eegTask === "nback" ? "text-orange-500" : eegTask === "resting" ? "text-slate-400" : "text-amber-500"}`}
                          >
                            {eegTask === "resting"
                              ? "MINIMAL"
                              : eegTask === "interpretable"
                        ...
                        <div
                          className={`w-full h-2 rounded-full overflow-hidden ${isDark ? "bg-slate-900" : "bg-slate-200"}`}
                        >
                          <div
                      ...
                      <div className="grid grid-cols-2 gap-3 pt-2">
                        <div
                          className={`p-2.5 rounded-xl ${isDark ? "bg-slate-950" : "bg-white border border-slate-100"}`}
                        >
                          <div className="text-[10px] font-mono text-slate-400 uppercase">
                            MODEL CONFIDENCE
                          </div>
                          <div
                            className={`text-base font-mono font-medium mt-0.5 ${isDark ? "text-slate-200" : "text-slate-800"}`}
                          >
                  ...
                  <p className="text-[11px] font-mono text-slate-400 leading-relaxed pt-2 border-t border-slate-800/10 mt-2">
                    ...
                    <strong>George&apos;s Findings:</strong> Transformers map
                    attention directly onto temporal and frontal electrode
                    groupings, verifying neural substrates for cognitive
                    switching. Overperforms standard CNN/LSTM models by 7-12% on
                    average.
                  </p>
                </div>
              </div>
          ...
          {activeTab === "bilingual" && (
            ...
            >
              <div className="lg:col-span-5 space-y-6">
                <div>
                  <h4
                    className={`text-sm font-mono font-medium mb-2 uppercase tracking-wider ${
                      isDark ? "text-amber-400" : "text-orange-600"
                    }`}
                  >
                  ...
                  <p
                    className={`text-xs mb-3 ${isDark ? "text-slate-400" : "text-slate-400"}`}
                  >
                    Type a bilingual mixed-language sentence to simulate the
                    Anterior Cingulate Cortex control costs. Switch between
                    Spanish words to observe cost spikes.
                  </p>
                  <textarea
                    value={inputText}
                  ...
                  />
                  <div
                    className={`mt-2 p-3 min-h-[3rem] rounded-xl border ${isDark ? "bg-slate-900 border-slate-800" : "bg-white border-slate-200"}`}
                  >
                    ...
                    <div className="text-xs font-sans flex flex-wrap gap-x-1 gap-y-1.5">
                      {inputText.split(/([\s,.]+)/).map((word, i) => {
                        ...
                        const isSpanish = SPANISH_WORDS.includes(
                        ...
                        return (
                          <span
                            key={i}
                            className={`px-1 rounded transition-colors duration-300 ${isSpanish ? (isDark ? "bg-amber-500/20 text-amber-400" : "bg-orange-500/20 text-orange-700 font-medium") : isDark ? "text-slate-300" : "text-slate-600"}`}
                          >
                            {word}
                          </span>
                        );
                ...
                <div>
                  ...
                  <div className="flex flex-col gap-1.5">
                    {[
                      ...
                      >
                        <span className="font-medium font-mono text-[10px] block">
                          {sample.label}
                        </span>
              ...
              <div className="lg:col-span-7 flex flex-col justify-between">
                ...
                >
                  <div className="flex items-center justify-between mb-4 pb-3 border-b border-dashed border-slate-800/20">
                    <span className="text-xs font-mono text-slate-400">
                      ANTERIOR CINGULATE CORTEX (sACC) MODEL
                    </span>
                  ...
                  <div className="h-32 w-full flex items-end relative pb-4 pt-2">
                    ...
                    >
                      ...
                      <motion.path
                        ...
                        fill="none"
                        stroke={
                          isDark ? "rgb(245, 158, 11)" : "rgb(249, 115, 22)"
                        }
                        strokeWidth="2.5"
                      />
                      {/* Dots on points */}
                      {switchTrajectory.map((val, idx) => (
                        <motion.circle
                          ...
                          r="4"
                          fill={
                            isDark ? "rgb(251, 146, 60)" : "rgb(249, 115, 22)"
                          }
                        />
                      ))}
                    </svg>

                    <span className="absolute text-[8px] font-mono text-slate-400 bottom-0 left-0">
                      START
                    </span>
                    <span className="absolute text-[8px] font-mono text-slate-400 bottom-0 right-0">
                      END OF SENTENCE
                    </span>
                    <span className="absolute text-[8px] font-mono text-slate-400 top-0 left-0">
                      1.0 sACC ACTIVATION
                    </span>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-4 mt-4 pt-4 border-t border-slate-800/10">
                    ...
                    <div>
                      <div className="text-[10px] font-mono text-slate-400">
                        SWITCH TRAJECTORY PATH
                      </div>
                      <div
                        className={`text-xs font-sans font-medium mt-2 ${saccCost > 0.4 ? "text-amber-500" : "text-emerald-500"}`}
                      >
                        {saccCost > 0.6
                  ...
                  <p className="text-[11px] font-mono text-slate-400 leading-relaxed mt-4 pt-2 border-t border-slate-800/10">
                    <strong>George&apos;s Insight:</strong> Multilingual BERT
                    layers verify that code-switching is not an abrupt jump, but
                    rather a smooth continuous activation shift across the
                    Anterior Cingulate Cortex, presenting symmetrical control
                    costs.
                  </p>
                </div>
              </div>
            ...
            >
              ...
              <div className="lg:col-span-5 space-y-6">
                <div>
                  ...
                  <p
                    className={`text-xs mb-4 ${isDark ? "text-slate-400" : "text-slate-400"}`}
                  >
                    Evaluate multi-agent reinforcement learning (MARL)
                    cooperation dynamics. Under noisy conditions, agents require
                    memory to build stable trust hierarchies.
                  </p>

                  <div className="space-y-4">
                    <div className="flex items-center justify-between p-3 rounded-xl border border-dashed border-slate-800/10">
                      <div>
                        <span className="text-xs font-sans font-medium block">
                          Noisy Environmental Observations
                        </span>
                        <span className="text-[10px] font-mono text-slate-400">
                          Simulate sensory/packet loss
                        </span>
                      </div>
                      <button
              ...
              <div className="lg:col-span-7 flex flex-col justify-between">
                ...
                >
                  <div className="flex items-center justify-between mb-4 pb-3 border-b border-dashed border-slate-800/20">
                    <span className="text-xs font-mono text-slate-400">
                      SOCIAL HIERARCHIES REAL-TIME AGENT MAP
                    </span>
                    <span className="text-xs font-mono flex items-center gap-1 text-slate-400 font-medium">
                      STATE:
                      <span
                        className={`font-mono text-[10px] ${agentStatus === "Converged" ? "text-emerald-500" : "text-amber-500"}`}
                      >
                        {agentStatus}
                      </span>
                    </span>
                  ...
                  <div
                    className={`h-36 w-full rounded-xl overflow-hidden relative flex items-center justify-center ${
                      isDark
                        ? "bg-slate-950"
                  ...
                  >
                    ...
                    <div className="relative w-full h-full">
                      ...
                      <motion.div
                        ...
                        transition={{
                          repeat: Infinity,
                          duration: agentStatus === "Desynchronized" ? 1 : 6,
                          ease: "easeInOut",
                        }}
                      ...
                      <motion.div
                        ...
                        transition={{
                          repeat: Infinity,
                          duration: agentStatus === "Desynchronized" ? 0.8 : 5,
                          ease: "easeInOut",
                        }}
                      ...
                      <motion.div
                        ...
                        transition={{
                          repeat: Infinity,
                          duration: agentStatus === "Desynchronized" ? 1.2 : 7,
                          ease: "easeInOut",
                        }}
                  ...
                  <div className="grid grid-cols-2 gap-4 mt-4 pt-4 border-t border-slate-800/10">
                    <div>
                      ...
                      <div
                        className={`text-xl font-mono font-medium mt-1 ${coordinationRate > 70 ? "text-emerald-500" : coordinationRate > 40 ? "text-amber-500" : "text-rose-500"}`}
                      >
                        {coordinationRate}%
                      </div>
                    </div>
                    <div>
                      ...
                      <div
                        className={`text-xl font-mono font-medium mt-1 ${trustLevel > 70 ? "text-emerald-500" : trustLevel > 40 ? "text-amber-500" : "text-rose-500"}`}
                      >
                        {trustLevel}%
                      </div>
                    </div>
                  ...
                  <p className="text-[11px] font-mono text-slate-400 leading-relaxed mt-4 pt-2 border-t border-slate-800/10">
                    <strong>George&apos;s Finding:</strong> Robustness testing
                    proves that Standard DQN drops coordination to 28% under
                    partial observation noise. LSTM-DQN variants bypass noise
                    limitations by forming stable temporal credit
                    representations (p = 0.0034).
                  </p>
                </div>
              </div>
            </motion.div>
            ...
            >
              ...
              <div className="lg:col-span-5 space-y-6">
                <div>
                  ...
                  <p
                    className={`text-xs mb-4 ${isDark ? "text-slate-400" : "text-slate-400"}`}
                  >
                    Evaluate multi-armed bandit agents modeled with classic
                    human cognitive biases to observe how irrational heuristic
                    behaviors impact decision-making.
                  </p>

                  <div className="space-y-4">
                    <div>
                      <span className="text-[10px] font-mono text-slate-400 uppercase block mb-2">
                        SELECT AGENT BIAS PROFILE
                      </span>
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 mb-4">
                        {[
                          {
                            id: "Normal",
                            desc: "Standard Epsilon-Greedy",
                            detail:
                              "A standard agent that balances exploration and exploitation without subjective weightings.",
                          },
                          {
                            id: "Loss-Averse",
                            desc: "Penalizes losses heavily",
                            detail:
                              "Inspired by Prospect Theory. Weighs failures (0) more negatively than successes (1).",
                          },
                          {
                            id: "Anchoring",
                            desc: "Overvalues initial results",
                            detail:
                              "Gives disproportionate weight to early experiences. Can get stuck if early pulls are bad.",
                          },
                          {
                            id: "Confirmation",
                            desc: "Ignores conflicting data",
                            detail:
                              "Only updates beliefs when new feedback agrees with expectations. Often clings to false beliefs.",
                          },
                          {
                            id: "Optimistic",
                            desc: "Overestimates initial values",
                            detail:
                              "Starts by assuming every arm is great, simulating overconfidence. Fuels early exploration.",
                          },
                        ].map((bias) => (
                              ...
                              <AnimatePresence>
                                ...
                                >
                                  <div className="pt-2 pb-1 px-1">
                                    <p
                                      className={`text-[10px] leading-relaxed ${isDark ? "text-slate-300" : "text-slate-600"}`}
                                    >
                                      {expandedBias
                                        ? bias.detail
                                        : `${bias.detail.substring(0, 45)}...`}
                                    </p>
                                    <button
                                      onClick={() =>
                                        setExpandedBias(!expandedBias)
                                      }
              ...
              <div className="lg:col-span-7 flex flex-col justify-between">
                ...
                >
                    ...
                    <span className="text-xs font-mono flex items-center gap-1 text-slate-400 font-medium">
                      ...
                      <span
                        className={`font-mono text-[10px] ${
                          biasStatus.includes("Failed") ||
                          biasStatus.includes("Stuck")
                            ? "text-rose-500"
                            : biasStatus.includes("Convergence") ||
                                biasStatus === "Optimal Avoidance" ||
                                biasStatus === "Converged"
                              ? "text-emerald-500"
                              : "text-amber-500"
                        }`}
                      >
                  ...
                  <div
                    className={`h-36 w-full rounded-xl overflow-hidden relative flex flex-col items-center justify-center ${
                      isDark
                        ? "bg-slate-950"
                        : "bg-white border border-slate-100"
                    }`}
                    ...
                    <div className="w-full px-8 space-y-6 z-10 relative">
                      <div>
                        <div className="flex justify-between text-[10px] font-mono text-slate-400 mb-2">
                          ...
                          <span
                            className={`font-medium ${winRate > 80 ? "text-emerald-500" : winRate > 50 ? "text-amber-500" : "text-rose-500"}`}
                          >
                            {winRate}%
                          </span>
                        </div>
                      ...
                      <div>
                        <div className="flex justify-between text-[10px] font-mono text-slate-400 mb-2">
                          ...
                          <span
                            className={`font-medium ${explorationRate > 60 ? "text-blue-400" : "text-slate-400"}`}
                          >
                            {explorationRate}%
                          </span>
                        </div>
                  ...
                  <p className="text-[11px] font-mono text-slate-400 leading-relaxed mt-4 pt-4 border-t border-slate-800/10">
                    ...
                    {activeBias === "Normal" &&
                      " The standard agent balances exploration and exploitation efficiently over time."}
                    {activeBias === "Loss-Averse" &&
                      " Loss aversion helps agents ditch low-reward arms faster, speeding up convergence to optimal strategies."}
                    {activeBias === "Anchoring" &&
                      " Anchoring on early poor rewards causes agents to get stuck in local optima, unable to unlearn first impressions."}
                    {activeBias === "Confirmation" &&
                      " Confirmation bias is detrimental, as agents cling to false beliefs and reject corrective feedback."}
                    {activeBias === "Optimistic" &&
                      " Optimism bias fuels early exploration, helping the agent find the best arm faster than a neutral approach."}
                  </p>
                </div>
              </div>
            </motion.div>
